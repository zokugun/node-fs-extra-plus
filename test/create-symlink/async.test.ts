...
const TEST_DIR = path.join(os.tmpdir(), 'test-fs-extra', 'create-symlink-async');
...
function fileSuccess(args: [string, string]) { // {{{
	...
	it(`should create symlink file using src ${srcpath} and dst ${dstpath}`, async () => {
		const result = await fse.createSymlinkAsync(...args);
...
function fileBroken(args: [string, string]) { // {{{
	...
	it(`should create broken symlink file using src ${srcpath} and dst ${dstpath}`, async () => {
		const result = await fse.createSymlinkAsync(...args);
		expect(result.fails).to.be.false;
...
function fileError(args: [string, string]) { // {{{
	...
	it(`should return error when creating symlink file using src ${srcpath} and dst ${dstpath}`, async () => {
...
function dirSuccess(args: [string, string]) { // {{{
	...
	it(`should create symlink dir using src ${srcpath} and dst ${dstpath}`, async () => {
		const result = await fse.createSymlinkAsync(...args);
		expect(result.fails).to.be.false;
...
function dirBroken(args: [string, string]) { // {{{
	...
	it(`should create broken symlink dir using src ${srcpath} and dst ${dstpath}`, async () => {
		const result = await fse.createSymlinkAsync(...args);
		expect(result.fails).to.be.false;
...
function dirError(args: [string, string]) { // {{{
	...
	it(`should return error when creating symlink dir using src ${srcpath} and dst ${dstpath}`, async () => {
		const dstdirExistsBefore = fs.existsSync(path.dirname(dstpath));
