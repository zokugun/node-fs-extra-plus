...
const TEST_DIR = path.join(os.tmpdir(), 'test-fs-extra', 'touch-sync');
...
it('file', async () => {
	...
	const touchResult = fse.touchSync(file);
	...
	const retouchResult = fse.touchSync(file);
...
it('dir', async () => {
	...
	const retouchResult = fse.touchSync(dir);
	expect(retouchResult.fails).to.be.false;
