...
const TEST_DIR = path.join(os.tmpdir(), 'test-fs-extra', 'write-json-async');
...
describe('spaces', () => {
	it('should read a file and parse the json', async () => {
		...
		await fse.writeJsonAsync(file, object);
...
describe('EOL', () => {
	it('should read a file and parse the json', async () => {
		...
		await fse.writeJsonAsync(file, object, { spaces: 2, EOL: '\r\n' });
