...
const TEST_DIR = path.join(os.tmpdir(), 'test-fs-extra', 'create-link-async');
...
function fileSuccess(args: [string, string]) { // {{{
	...
	it(`should create link file using src ${srcpath} and dst ${dstpath}`, async () => {
		const result = await fse.createLinkAsync(...args);
...
function fileError(args: [string, string]) { // {{{
	...
	it(`should return error when creating link file using src ${srcpath} and dst ${dstpath}`, async () => {
