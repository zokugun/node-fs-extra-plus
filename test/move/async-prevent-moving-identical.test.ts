...
const TEST_DIR = path.join(os.tmpdir(), 'test-fs-extra', 'move-async-prevent-moving-identical');
...
it('should return an error if src and dest are the same', async () => {
	const fileSource = path.join(TEST_DIR, 'TEST_fs-extra_move_sync');
	const fileDestination = path.join(TEST_DIR, 'TEST_fs-extra_move_sync');
	fse.ensureFileSync(fileSource);

	const result = await fse.moveAsync(fileSource, fileDestination);
...
describe('dest with parent symlink', () => {
	describe('first parent is symlink', () => {
		...
		it('should error when src is directory', async () => {
...
describe('> when src is a directory', () => {
	describe('>> when src is regular and dest is a symlink that points to src', () => {
		it('should error if dereferene is true', async () => {
	...
	describe('>> when src is a symlink that points to a regular dest', () => {
		it('should throw error', async () => {
			...
			fs.symlinkSync(destination, sourceLink, 'dir');
	...
	describe('>> when src and dest are symlinks that point to the exact same path', () => {
		it('should error if src and dest are the same and dereference is true', async () => {
			const source = path.join(TEST_DIR, 'src');
...
describe('> when src is a file', () => {
	describe('>> when src is regular and dest is a symlink that points to src', () => {
		it('should error if dereference is true', async () => {
