...
// const $renameErrorCode: string | null = null;
// let $renameCallCount = 0;

// vi.doMock('../../src/fs/async.js', async (factory) => {
// 	const original: { rename: (...args) => Promise<FsVoidResult> } = await factory();
...
const TEST_DIR = path.join(os.tmpdir(), 'test-fs-extra', 'move-async');
...
it('should not move if src and dest are the same', async () => {
...
it('should error if src and dest are the same and src does not exist', async () => {
...
it('should rename a file on the same device', async () => {
	const source = `${TEST_DIR}/a-file`;
	const destination = `${TEST_DIR}/a-file-dest`;

	await fse.moveAsync(source, destination);
...
it('should not overwrite the destination by default', async () => {
	const source = `${TEST_DIR}/a-file`;
...
it('should not overwrite if overwrite = false', async () => {
	const source = `${TEST_DIR}/a-file`;
	const destination = `${TEST_DIR}/a-folder/another-file`;
	...
	const result = await fse.moveAsync(source, destination, { overwrite: false });
	expect(result.fails).to.be.true;
	expect(result.error!.message).to.equals('dest already exists.');
});

it('should overwrite file if overwrite = true', async () => {
	const source = `${TEST_DIR}/a-file`;
	const destination = `${TEST_DIR}/a-folder/another-file`;
	...
	const result = await fse.moveAsync(source, destination, { overwrite: true });
...
it('should overwrite the destination directory if overwrite = true', async () => {
	// Create src
	...
	await fse.moveAsync(source, destination, { overwrite: true });
...
it('should create directory structure by default', async () => {
	const source = `${TEST_DIR}/a-file`;
...
// it('should work across devices', async () => {
// 	const src = `${TEST_DIR}/a-file`
// 	const dest = `${TEST_DIR}/a-file-dest`

// 	setUpMockFs('EXDEV')

// 	await fse.moveAsync(src, dest)
...
// it('should overwrite folders across devices', async () => {
...
// 	await fse.moveAsync(src, dest, { overwrite: true })
...
// it('should move folders across devices with EXDEV error', async () => {
// 	const src = `${TEST_DIR}/a-folder`
// 	const dest = `${TEST_DIR}/a-folder-dest`
...
describe('clobber', () => {
	it('should be an alias for overwrite', async () => {
		const source = `${TEST_DIR}/a-file`;
		const destination = `${TEST_DIR}/a-folder/another-file`;
		...
		await fse.moveAsync(source, destination, { clobber: true });
...
describe('> when trying to move a folder into itself', () => {
	it('should produce an error', async () => {
		...
		const result = await fse.moveAsync(sourceDir, destinationDir);
		expect(result.fails).to.be.true;
...
describe.runIf(process.platform === 'win32')('> when dest parent is root', () => {
	...
	it('should not create parent directory', async () => {
