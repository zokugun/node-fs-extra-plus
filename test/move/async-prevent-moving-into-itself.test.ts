...
const TEST_DIR = path.join(os.tmpdir(), 'test-fs-extra', 'move-async-case-insensitive-paths');
const FILES = [
	'file0.txt',
...
describe('> when source is a file', () => {
	it('should move the file successfully even if dest parent is a subdir of src', async () => {
		...
		const result = await fse.moveAsync(sourceFile, destinationFile);
...
describe('> when source is a file', () => {
	it('should move the file successfully even when dest parent is \'src/dest\'', async () => {
		const destinationFile = path.join(TEST_DIR, 'src', 'dest', 'destfile.txt');
		return testSuccessFile(source, destinationFile);
	});

	it('should move the file successfully when dest parent is \'src/src_dest\'', async () => {
		const destinationFile = path.join(TEST_DIR, 'src', 'src_dest', 'destfile.txt');
		return testSuccessFile(source, destinationFile);
	});

	it('should move the file successfully when dest parent is \'src/dest_src\'', async () => {
		const destinationFile = path.join(TEST_DIR, 'src', 'dest_src', 'destfile.txt');
		return testSuccessFile(source, destinationFile);
	});

	it('should move the file successfully when dest parent is \'src/dest/src\'', async () => {
		const destinationFile = path.join(TEST_DIR, 'src', 'dest', 'src', 'destfile.txt');
		return testSuccessFile(source, destinationFile);
	});

	it('should move the file successfully when dest parent is \'srcsrc/dest\'', async () => {
...
describe('> when source is a directory', () => {
	describe('>> when dest is a directory', () => {
		...
		it('should move the directory successfully when dest is \'src_dest\'', async () => {
			const destination = path.join(TEST_DIR, 'src_dest');
			return testSuccessDir(source, destination);
		});
		it('should move the directory successfully when dest is \'src-dest\'', async () => {
			const destination = path.join(TEST_DIR, 'src-dest');
			return testSuccessDir(source, destination);
		});

		it('should move the directory successfully when dest is \'dest_src\'', async () => {
			const destination = path.join(TEST_DIR, 'dest_src');
			return testSuccessDir(source, destination);
		});

		it('should move the directory successfully when dest is \'src_dest/src\'', async () => {
			const destination = path.join(TEST_DIR, 'src_dest', 'src');
			return testSuccessDir(source, destination);
		});

		it('should move the directory successfully when dest is \'src-dest/src\'', async () => {
			const destination = path.join(TEST_DIR, 'src-dest', 'src');
			return testSuccessDir(source, destination);
		});

		it('should move the directory successfully when dest is \'dest_src/src\'', async () => {
			const destination = path.join(TEST_DIR, 'dest_src', 'src');
			return testSuccessDir(source, destination);
		});

		it('should move the directory successfully when dest is \'src_src/dest\'', async () => {
			const destination = path.join(TEST_DIR, 'src_src', 'dest');
			return testSuccessDir(source, destination);
		});

		it('should move the directory successfully when dest is \'src-src/dest\'', async () => {
			const destination = path.join(TEST_DIR, 'src-src', 'dest');
			return testSuccessDir(source, destination);
		});

		it('should move the directory successfully when dest is \'srcsrc/dest\'', async () => {
			const destination = path.join(TEST_DIR, 'srcsrc', 'dest');
			return testSuccessDir(source, destination);
		});

		it('should move the directory successfully when dest is \'dest/src\'', async () => {
			const destination = path.join(TEST_DIR, 'dest', 'src');
			return testSuccessDir(source, destination);
		});

		it('should move the directory successfully when dest is very nested that all its parents need to be created', async () => {
		...
		it('should error when dest is \'src/dest\'', async () => {
			const destination = path.join(TEST_DIR, 'src', 'dest');
			return testError(source, destination);
		});

		it('should error when dest is \'src/src_dest\'', async () => {
			const destination = path.join(TEST_DIR, 'src', 'src_dest');
			return testError(source, destination);
		});

		it('should error when dest is \'src/dest_src\'', async () => {
			const destination = path.join(TEST_DIR, 'src', 'dest_src');
			return testError(source, destination);
		});

		it('should error when dest is \'src/dest/src\'', async () => {
			const destination = path.join(TEST_DIR, 'src', 'dest', 'src');
			return testError(source, destination);
		});
	});
	...
	describe('>> when dest is a symlink', () => {
		it('should error when dest points exactly to src and dereference is true', async () => {
			...
			const result = await fse.moveAsync(source, destinationLink, { dereference: true });
		...
		it('should error when dest is a subdirectory of src (bind-mounted directory with subdirectory)', async () => {
			const destinationLink = path.join(TEST_DIR, 'dest-symlink');
			fs.symlinkSync(source, destinationLink, 'dir');
			...
			const result = await fse.moveAsync(source, destination);
			expect(result.fails).to.be.true;
		...
		it('should error when dest is a subdirectory of src (more than one level depth)', async () => {
			const destinationLink = path.join(TEST_DIR, 'dest-symlink');
			fs.symlinkSync(source, destinationLink, 'dir');
...
describe('> when source is a symlink', () => {
	describe('>> when dest is a directory', () => {
		it('should error when resolved src path points to dest', async () => {
			...
			const result = await fse.moveAsync(sourceLink, destination);
			expect(result.fails).to.be.true;
		...
		it('should error when dest is a subdir of resolved src path', async () => {
			const sourceLink = path.join(TEST_DIR, 'src-symlink');
			fs.symlinkSync(source, sourceLink, 'dir');
		...
		it('should error when resolved src path is a subdir of dest', async () => {
		...
		it('should move the directory successfully when dest is \'src_src/dest\'', async () => {
			...
			const destination = path.join(TEST_DIR, 'src_src', 'dest');
			await testSuccessDir(sourceLink, destination);
	...
	describe('>> when dest is a symlink', () => {
		it('should error when resolved dest path is exactly the same as resolved src path and dereferene is true', async () => {
			const sourceLink = path.join(TEST_DIR, 'src-symlink');
			fs.symlinkSync(source, sourceLink, 'dir');
			const destinationLink = path.join(TEST_DIR, 'dest-symlink');
			fs.symlinkSync(source, destinationLink, 'dir');
			...
			const result = await fse.moveAsync(sourceLink, destinationLink, { dereference: true });
			expect(result.fails).to.be.true;
			expect(result.error!.message).to.equals('Source and destination must not be the same.');
...
async function testSuccessFile(source: string, destinationFile: string) {
...
async function testSuccessDir(source: string, destination: string) {
...
async function testError(source: string, destination: string) {
	const result = await fse.moveAsync(source, destination);
	expect(result.fails).to.be.true;
	expect(result.error!.message).to.equals(`Cannot move '${source}' to a subdirectory of itself, '${destination}'.`);
