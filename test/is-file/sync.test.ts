...
const TEST_DIR = path.join(os.tmpdir(), 'test-fs-extra', 'is-file-sync');
...
it('ok - empty', () => {
	...
	const result = fse.isFileSync(path.join(TEST_DIR, 'some-file.txt'));
...
it('fails - dir', () => {
	...
	const result = fse.isFileSync(path.join(TEST_DIR, 'some-dir'));
