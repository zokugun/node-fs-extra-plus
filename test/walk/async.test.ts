...
it('should return an error if the source is not a dir', async () => {
	const result = await fse.walkAsync(FILE_PATHS[0]);

	expect(result.fails).to.be.true;
	expect(result.error!.code).to.equals('ENOTDIR');
});

it('should return all items of a dir containing path and stats object', async () => {
	const paths = [
	...
	const result = await fse.walkAsync(TEST_DIR);
...
it('should return only files if opts.onlyFiles is true', async () => {
	const filesOnly = [
		{ path: FILE_NAMES[0], stats: fs.statSync(FILE_PATHS[0]) },
		{ path: FILE_NAMES[1], stats: fs.statSync(FILE_PATHS[1]) },
		{ path: FILE_NAMES[2], stats: fs.statSync(FILE_PATHS[2]) },
	...
	const result = await fse.walkAsync(TEST_DIR, { onlyFiles: true });
	expect(result.fails).to.be.false;
...
it('should return only dirs if opts.onlyDirectories is true', async () => {
	...
	const result = await fse.walkAsync(TEST_DIR, { onlyDirectories: true });
	expect(result.fails).to.be.false;
...
it('should honor opts.absolute is true', async () => {
	const paths = [
	...
	const result = await fse.walkAsync(TEST_DIR, { absolute: true });
	expect(result.fails).to.be.false;
...
it('should honor opts.markDirectories is true', async () => {
	const paths = [
	...
	const result = await fse.walkAsync(TEST_DIR, { markDirectories: true });
	expect(result.fails).to.be.false;
...
it('should honor opts.onlyFiles is true and with sorter', async () => {
	const filesOnly = [
		{ path: FILE_NAMES[2], stats: fs.statSync(FILE_PATHS[2]) },
		{ path: FILE_NAMES[1], stats: fs.statSync(FILE_PATHS[1]) },
		{ path: FILE_NAMES[0], stats: fs.statSync(FILE_PATHS[0]) },
	...
	const result = await fse.walkAsync(TEST_DIR, { onlyFiles: true, sorter: (a, b) => b.localeCompare(a) });
	expect(result.fails).to.be.false;
...
describe('when opts.filter is true', () => {
	it('should filter based on path', async () => {
		...
		const result = await fse.walkAsync(TEST_DIR, { filter });
		expect(result.fails).to.be.false;
	...
	it('should filter based on stats', async () => {
	...
	it('should filter based on both path and stats', async () => {
		const f1 = path.join(TEST_DIR, 'foo.js');
		const f2 = path.join(TEST_DIR, 'bar.js');
		fse.outputFileSync(f1, 'test file 1 contents');
		fse.outputFileSync(f2, 'test file 2 contents');
	...
	it('should filter and apply opts.onlyFiles', async () => {
		const f1 = path.join(TEST_DIR, 'bar.js');
		const f2 = path.join(TEST_DIR, 'foo.js');
		fse.outputFileSync(f1, 'test file 1 contents');
		fse.outputFileSync(f2, 'test file 2 contents');
		...
		const result = await fse.walkAsync(TEST_DIR, { filter, onlyFiles: true });
		expect(result.fails).to.be.false;
	...
	it('should filter and apply opts.onlyDirectories', async () => {
		...
		const result = await fse.walkAsync(TEST_DIR, { filter, onlyDirectories: true });
		expect(result.fails).to.be.false;
...
describe('traverse all', () => {
	async function testTraverseAll(options: WalkOptions, expected) {
		...
		const result = await fse.walkAsync(TEST_DIR, { ...options, traverseAll: true });
		expect(result.fails).to.be.false;
...
describe('symlinks', () => {
	async function testSymlinks(preserveSymlinks: boolean, expected) {
		...
		const result = await fse.walkAsync(TEST_DIR, { preserveSymlinks });
		expect(result.fails).to.be.false;
...
describe('collect', () => {
	it('should return all items of a dir containing path and stats object', async () => {
		...
		const result = await fse.walkAsync(TEST_DIR, { collect: true });
		expect(result.fails).to.be.false;
