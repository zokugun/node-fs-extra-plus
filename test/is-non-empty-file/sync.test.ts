...
const TEST_DIR = path.join(os.tmpdir(), 'test-fs-extra', 'is-non-empty-file-sync');
...
it('fails - empty', () => {
	...
	const result = fse.isNonEmptyFileSync(path.join(TEST_DIR, 'some-file.txt'));
...
it('fails - dir', () => {
	...
	const result = fse.isNonEmptyFileSync(path.join(TEST_DIR, 'some-dir'));
