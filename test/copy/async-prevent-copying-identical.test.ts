...
const TEST_DIR = path.join(os.tmpdir(), 'test-fs-extra', 'copy-async-prevent-copying-identical');
...
it('should return an error if src and dest are the same', async () => {
	...
	const result = await fse.copyAsync(fileSource, fileDestination);
...
describe('> when src is a directory', () => {
	...
	describe('>> when src and dest are symlinks that point to the exact same path', () => {
		it('should error if src and dest are the same and dereferene is true', async () => {
			const source = path.join(TEST_DIR, 'src');
