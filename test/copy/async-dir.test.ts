...
const TEST_DIR = path.join(os.tmpdir(), 'test-fs-extra', 'copy-async-dir');
...
describe('> when src is a directory', () => {
	...
	it('should copy the directory synchronously', async () => {
		const files = 2;
...
describe('> when filter is used', () => {
	it('should do nothing if filter fails', async () => {
		...
		const filter = async (s: string) => path.extname(s) !== '.css' && !fs.statSync(s).isDirectory();

		await fse.copyAsync(sourceFile, destinationFile, filter);
	...
	it('should apply filter recursively', async () => {
		...
		const filter = async (s: string) => /(0|\D)$/i.test(s);
	...
	it('should apply the filter to directory names', async () => {
		const ignore = 'ignore';
		const filter = async (s: string) => s.includes(ignore);
	...
	it('should apply filter when it is applied only to dest', async () => {
		...
		const filter = async (_s: string, d: string) => fs.statSync(d).mtime.getTime() < timeCond;
	...
	it('should apply filter when it is applied to both src and dest', async () => {
		const timeCond = Date.now();
		const filter = async (s: string, d: string) => s.split('.').pop() !== 'css' && fs.statSync(path.dirname(d)).mtime.getTime() > timeCond;
