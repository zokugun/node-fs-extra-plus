...
const TEST_DIR = path.join(os.tmpdir(), 'test-fs-extra', 'copy-async-prevent-copying-into-itself');
const FILES = [
	'file0.txt',
...
describe('> when source is a file', () => {
	it('should copy the file successfully even if dest parent is a subdir of src', async () => {
		...
		await fse.copyAsync(sourceFile, destinationFile);
...
describe('> when source is a directory', () => {
	describe('>> when dest is a directory', () => {
		...
		it('should copy the directory successfully when dest is \'src_dest\'', async () => {
			const destination = path.join(TEST_DIR, 'src_dest');
			return testSuccess(source, destination);
		});
		it('should copy the directory successfully when dest is \'src-dest\'', async () => {
			const destination = path.join(TEST_DIR, 'src-dest');
			return testSuccess(source, destination);
		});

		it('should copy the directory successfully when dest is \'dest_src\'', async () => {
			const destination = path.join(TEST_DIR, 'dest_src');
			return testSuccess(source, destination);
		});

		it('should copy the directory successfully when dest is \'src_dest/src\'', async () => {
			const destination = path.join(TEST_DIR, 'src_dest', 'src');
			return testSuccess(source, destination);
		});

		it('should copy the directory successfully when dest is \'src-dest/src\'', async () => {
			const destination = path.join(TEST_DIR, 'src-dest', 'src');
			return testSuccess(source, destination);
		});

		it('should copy the directory successfully when dest is \'dest_src/src\'', async () => {
			const destination = path.join(TEST_DIR, 'dest_src', 'src');
			return testSuccess(source, destination);
		});

		it('should copy the directory successfully when dest is \'src_src/dest\'', async () => {
			const destination = path.join(TEST_DIR, 'src_src', 'dest');
			return testSuccess(source, destination);
		});

		it('should copy the directory successfully when dest is \'src-src/dest\'', async () => {
			const destination = path.join(TEST_DIR, 'src-src', 'dest');
			return testSuccess(source, destination);
		});

		it('should copy the directory successfully when dest is \'srcsrc/dest\'', async () => {
			const destination = path.join(TEST_DIR, 'srcsrc', 'dest');
			return testSuccess(source, destination);
		});

		it('should copy the directory successfully when dest is \'dest/src\'', async () => {
			const destination = path.join(TEST_DIR, 'dest', 'src');
			return testSuccess(source, destination);
		});

		it('should copy the directory successfully when dest is very nested that all its parents need to be created', async () => {
	...
	describe('>> when dest is a symlink', () => {
		it('should error when dest points exactly to src and dereference is true', async () => {
			...
			const result = await fse.copyAsync(source, destinationLink, { dereference: true });
		...
		it('should copy the directory successfully when src is a subdir of resolved dest path and dereferene is true', async () => {
			const sourceInDestination = path.join(TEST_DIR, 'dest', 'some', 'nested', 'src');
			const destinationLink = path.join(TEST_DIR, 'dest-symlink');
			await fse.copyAsync(source, sourceInDestination); // put some stuff in srcInDest
			...
			await fse.copyAsync(sourceInDestination, destinationLink, { dereference: true });
...
describe('> when source is a symlink', () => {
	describe('>> when dest is a directory', () => {
		it('should error when resolved src path points to dest', async () => {
			...
			const result = await fse.copyAsync(sourceLink, destination);
			expect(result.fails).to.be.true;
		...
		it('should error when resolved src path is a subdir of dest', async () => {
			...
			const sourceLink = path.join(TEST_DIR, 'src-symlink');
			await fse.copyAsync(source, resolvedSourcePath);
		...
		it('should copy the directory successfully when dest is \'src_src/dest\'', async () => {
			...
			await testSuccess(sourceLink, destination);
	...
	describe('>> when dest is a symlink', () => {
		it('should error when resolved dest path is exactly the same as resolved src path and dereference is true', async () => {
			const sourceLink = path.join(TEST_DIR, 'src-symlink');
			fse.symlinkSync(source, sourceLink, 'dir');
			const destinationLink = path.join(TEST_DIR, 'dest-symlink');
			fse.symlinkSync(source, destinationLink, 'dir');
			...
			const result = await fse.copyAsync(sourceLink, destinationLink, { dereference: true });
			expect(result.fails).to.be.true;
			expect(result.error!.message).to.equals('Source and destination must not be the same.');
		...
		it('should error when resolved dest path is a subdir of resolved src path', async () => {
			const sourceLink = path.join(TEST_DIR, 'src-symlink');
			fs.symlinkSync(source, sourceLink, 'dir');
		...
		it('should error when resolved src path is a subdir of resolved dest path', async () => {
			const sourceInDestination = path.join(TEST_DIR, 'dest', 'some', 'nested', 'src');
			const sourceLink = path.join(TEST_DIR, 'src-symlink');
			const destinationLink = path.join(TEST_DIR, 'dest-symlink');
			const destination = path.join(TEST_DIR, 'dest');
...
async function testSuccess(source: string, destination: string) {
	const srclen = klawSync(source).length;

	expect(srclen).toBeGreaterThan(2);

	await fse.copyAsync(source, destination);
