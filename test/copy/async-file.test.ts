...
const TEST_DIR = path.join(os.tmpdir(), 'test-fs-extra', 'copy-async-file');
...
it('should copy the file synchronously', async () => {
	...
	await fse.copyAsync(fileSource, fileDestination);
...
it('should follow symlinks', async () => {
	...
	await fse.copyAsync(linkSource, fileDestination);
...
it('should maintain file mode', async () => {
	const fileSource = path.join(TEST_DIR, 'TEST_fs-extra_src');
	const fileDestination = path.join(TEST_DIR, 'TEST_fs-extra_copy');
...
it('should only copy files allowed by filter fn', async () => {
	...
	const filter = async (s: string) => s.split('.').pop() !== 'css';

	await fse.copyAsync(sourceFile1, destinationFile1, filter);
	await fse.copyAsync(sourceFile2, destinationFile2, filter);
	await fse.copyAsync(sourceFile3, destinationFile3, filter);
...
it('should not call filter fn more than needed', async () => {
	...
	await fse.copyAsync(source, destination, filter);
...
describe('> when src file does not have write permissions', () => {
	it('should be able to copy contents of file', async () => {
...
describe('> when overwrite option is passed', () => {
	...
	describe('> when destination file does NOT exist', () => {
		describe('> when overwrite is true', () => {
			it('should copy the file and not throw an error', async () => {
				await fse.copyAsync(source, destination, { overwrite: true });
		...
		describe('> when overwrite is false', () => {
			it('should copy the file and not throw an error', async () => {
				await fse.copyAsync(source, destination, { overwrite: false });
	...
	describe('when destination file does exist', () => {
		...
		describe('> when overwrite is false', () => {
			...
			it('should throw an error when errorOnExist is true', async () => {
				const result = await fse.copyAsync(source, destination, { overwrite: false, errorOnExist: true });
				expect(result.fails).to.be.true;
...
describe('clobber', () => {
	...
	it('is an alias for overwrite', async () => {
		await fse.copyAsync(source, destination, { clobber: false });
