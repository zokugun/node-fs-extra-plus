...
const TEST_DIR = path.join(os.tmpdir(), 'test-fs-extra', 'copy-async-broken-symlink');
...
describe('when symlink is broken', () => {
	it('should not throw error if dereference is false', async () => {
		const result = await fse.copyAsync(source, destination);
		expect(result.fails).to.be.false;
	});

	it('should throw error if dereference is true', async () => {
		const result = await fse.copyAsync(source, destination, { dereference: true });
