...
const TEST_DIR = path.join(os.tmpdir(), 'test-fs-extra', 'copy-async-symlink');
...
it('copies symlinks by default', async () => {
	const result = await fse.copyAsync(source, out);
...
it('copies file contents when dereference=true', async () => {
	const result = await fse.copyAsync(source, out, { dereference: true });
	expect(result.fails).to.be.false;
