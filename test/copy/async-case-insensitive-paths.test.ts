...
const TEST_DIR = path.join(os.tmpdir(), 'test-fs-extra', 'copy-async-case-insensitive-paths');
...
describe('> when src is a directory', () => {
	it('should behave correctly based on the OS', async () => {
...
describe('> when src is a symlink', () => {
	it('should behave correctly based on the OS, symlink dir', async () => {
		const source = path.join(TEST_DIR, 'srcdir');
	...
	it('should behave correctly based on the OS, symlink file', async () => {
		const source = path.join(TEST_DIR, 'srcfile');
