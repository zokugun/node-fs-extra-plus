...
export function createSymlink(source: string, target: string, type?: fs.symlink.Type | null): Result<void, NodeJS.ErrnoException | Error> {
	...
	if(!stats.fails && stats.value.isSymbolicLink()) {
		const sourceStats = stat(source);
		if(sourceStats.fails) {
			return sourceStats;
		}

		const tgtStats = stat(target);
		if(tgtStats.fails) {
			return tgtStats;
		}
	...
	const relative = symlinkPaths(source, target);
	if(relative.fails) {
		return relative;
	}

	source = relative.value.toDst;
	type = symlinkType(relative.value.toCwd, type);

	const dir = path.dirname(target);

	const xstResult = exists(dir);
	if(xstResult.fails) {
		return xstResult;
	}
...
export function symlinkPaths(source: string, target: string): FsResult<{ toCwd: string; toDst: string }> {
	if(path.isAbsolute(source)) {
		const result = exists(source);
		if(result.fails) {
			return result;
		}
	...
	const xstResult = exists(relativeToTarget);
	if(xstResult.fails) {
		return xstResult;
	}
...
function symlinkType(file: string, type: fs.symlink.Type | null | undefined): fs.symlink.Type {
	if(type) {
		return type;
	}
