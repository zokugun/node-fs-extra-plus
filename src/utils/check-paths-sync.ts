...
function getStats(source: string, destination: string, options: { dereference?: boolean }): FsResult<{ srcStat: BigIntStats; destStat: BigIntStats | null }> {
	const statFn = options.dereference ? stat : lstat;

	const sourceStats = statFn(source, { bigint: true });
	if(sourceStats.fails) {
		return sourceStats;
	}

	const destinationStats = statFn(destination, { bigint: true });
